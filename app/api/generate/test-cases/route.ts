...
import { createClaudeStream, parseJson, buildSystemPrompt, BASE_SYSTEM_TC } from '@/lib/claude'
import { storage } from '@/lib/storage'
import { GeneratedDocument, InputType } from '@/lib/types'
import { buildTestCasePrompt } from '@/lib/prompts/test-cases'
...
export async function POST(request: Request) {
  ...
  // ── Setup phase (before streaming starts) ──
  ...
  try {
    if (contentType.includes('multipart/form-data')) {
      ...
      input = body.input || body.userStory
  ...
  const userPrompt = buildTestCasePrompt(
    ...
    async start(controller) {
      try {
        ...
        const doc: GeneratedDocument = {
          ...
          type: 'test-cases',
          inputType: inputType!,
          inputSummary: parsed.feature || 'Untitled Feature',
          version: 1,
          createdAt: new Date().toISOString(),
          content: parsed.testCases || [],
        ...
        console.error('[generate/test-cases]', msg)
